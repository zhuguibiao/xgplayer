...
import { EVENT, NetLoader } from 'xgplayer-streaming-shared'
import { FlvDemuxer } from 'xgplayer-transmuxer'
import { Flv } from './flv'
import { FlvH265 } from './flv/h265'
...
export class FlvPlugin extends BasePlugin {
  ...
  beforePlayerInit() {
    ...
    // 通过插件本身API销毁插件
    const startPlugin = this.player.getPlugin('play')
    startPlugin.__destroy()
    this.on(Events.URL_CHANGE, this._onSwitchURL)
    this.on(Events.VOLUME_CHANGE, (e) => {
      console.log(e)
    })
    this.on(Events.PAUSE, e => {
      console.log('pasue')
    })
