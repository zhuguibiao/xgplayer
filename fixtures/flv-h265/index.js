...
import FlvPlayer from '../../packages/xgplayer-h265-flv/src'
...
var opts = Object.assign(
  {
    // url: 'http://10.0.0.136:18000/flv/live/34020000001320000001_34020000001410000001_0200000001.flv'
    // url: 'http://127.0.0.1:8081/fixtures/flv-h265/h265.flv',
...
window.onload = function () {
  ...
  switch (testPoint) {
    ...
    case 1:
      {
        desc =
          '设置高亮四个参数。<br>查看"开发者工具"的Network面板是否进行相应次数的重试<br>查看“日志”是否触发相应次数的重试事件“core.loadretry”、请求开始“core.loadstart”和请求完成“core.loadcomplete”'
        desc +=
          '<br>拉流最大探测时间: 对于存在音、视频标识，但是一直拉取不到音、视频数据的case, 拉流数据超过设置的时长后当做无音频或者无视频起播'
    ...
    case 2:
      {
        desc =
          '设置高亮参数，并观察状态区域中的“当前时间之前缓存时长”是否与设置的值差不多（允许一定的误差）'
    ...
    case 3:
      {
        desc =
          '设置高亮两个参数，观察视频是否自动播放(自动播放失败，点击播放按钮可以正常播放)，如果是静音自动播放则必须自动播放成功'
    ...
    case 4:
      {
        desc =
          '点击 API 方法中的 “切换url” 和 “设置url” 按钮，观察是否切换视频成功，并且可以播放，暂停'
    ...
    case 5:
      {
        desc =
          '设置该参数，然后在直播中暂停，等待设置该参数差不多的时间，然后点播放按钮，观察直播是否拉流（出现黑屏）。等待小于该时间点播放可以连续播放不会重新拉流（不黑屏）' +
          '<br> 如果设置了无缝重拉流则不会黑屏'
  ...
  inp(doUrl).onchange = function () {
    updateOpts('url', this.value, 'string')
  }
  inp(doAutoplay).onchange = function () {
    updateOpts('autoplay', this.checked)
  }
  inp(doAutoplayMuted).onchange = function () {
    updateOpts('autoplayMuted', this.checked)
  }
  inp(doSeamlesslyReload).onchange = function () {
    updateOpts('seamlesslyReload', this.checked)
  }
  inp(doLoadTimeout).onchange = function () {
    updateOpts('loadTimeout', this.value, 'number')
  }
  inp(doRetryCount).onchange = function () {
    updateOpts('retryCount', this.value, 'number')
  }
  inp(doRetryDelay).onchange = function () {
    updateOpts('retryDelay', this.value, 'number')
  }
  inp(doAnalyzeDuration).onchange = function () {
    updateOpts('analyzeDuration', this.value, 'number')
  }
  inp(doFetchOptions).onchange = function () {
    updateOpts('fetchOptions', this.value, 'object')
  }
  inp(doBufferBehind).onchange = function () {
    updateOpts('bufferBehind', this.value, 'number')
  }
  inp(doMaxJumpDistance).onchange = function () {
    updateOpts('maxJumpDistance', this.value, 'number')
  }
  inp(doTargetLatency).onchange = function () {
    updateOpts('targetLatency', this.value, 'number')
  }
  inp(doMaxLatency).onchange = function () {
    updateOpts('maxLatency', this.value, 'number')
  }
  inp(doDisconnectTime).onchange = function () {
    updateOpts('disconnectTime', this.value, 'number')
  }
  inp(doMaxReaderInterval).onchange = function () {
    updateOpts('maxReaderInterval', this.value, 'number')
  }
  ...
  dbPlay.onclick = function () {
    player.play()
  }
  dbPause.onclick = function () {
    player.pause()
  }
  dbReplay.onclick = function () {
    player.replay()
  }
  ...
  //       if (lastPlayback) {
  //         fps = Math.round(
  //           ((pq.totalVideoFrames - lastPlayback.totalVideoFrames) /
  //             (pq.creationTime - lastPlayback.creationTime)) *
  //             1000
  //         )
  //       }
  //       lastPlayback = pq
